...
def calculate_confidence(points, n_total_override=None, suburb_key=None):
    ...
    # ── Range calculation ──────────────────────────────────────────────────
    # The published low/high guides are an EMPIRICAL 80% BAND: measured against
    # real sales, four in five land inside it. Decided by Will 2026-08-07 —
    # see 16_Valuation/decisions/2026-08-07-range-meaning.md.
    #
    # It replaced a flat ±12% that was described on two live pages as a "90%
    # confidence interval". It was never a confidence interval, and measured
    # against 641 real sales it contained the sale 58% of the time, not 90%.
    #
    # ⚠ THIS NUMBER IS AN OUTPUT OF THE BACKTEST, NOT A CONSTANT TO TUNE.
    # Re-measure with `scripts/valuation_backtest.py --price-filter none
    # --property-type House --min-price 1000000 --max-price 2000000` after ANY
    # change to the method, and update it. Improving the method is the only
    # legitimate way to narrow the band. If a measurement shows coverage below
    # 80%, the band WIDENS — it does not get quietly left alone.
    #
...
# ─── Suburb calibration + retired adjustments (added 2026-08-07) ──────────────
#
# WHY THESE EXIST
# The comparable-sales method runs systematically LOW, and by a different amount
# in each suburb. Measured 2026-08-07 over 588 real detached-house sales
# $1M–$2M, after the three subjective adjustments below were dropped:
#     Robina ........... -2.3%   (n=256)
#     Burleigh Waters .. -3.0%   (n=146)
#     Varsity Lakes ... -11.8%   (n=186)
# A single global correction leaves most of that on the table — Varsity Lakes is
# five times Robina's offset. Applying each suburb its own factor took the 80%
# band from ±16.3% to ±14.5% on off-market subjects.
#
# ⚠ RE-MEASURE THESE. They are empirical constants with a shelf life, not
# physics. Re-derive after any method change and whenever the market moves, with
#   scripts/valuation_backtest.py --price-filter none --property-type House \
#     --min-price 1000000 --max-price 2000000 --suburb <suburb> --blind-subject
# A suburb with no entry gets 1.0 — no correction, never a guess.
#
# ⚠ The Varsity Lakes offset is LARGE and is partly a data-coverage artefact:
# even its RAW comparable pool sits ~11% below eventual sale prices, consistent
...
# offset — removing each of these IMPROVED accuracy:
#     kitchen ............. -0.40pp MAE
#     renovation .......... -0.25pp
#     renovation_quality .. -0.25pp
# All three are AI-derived quality judgements read off marketing photographs.
# The physical, measurable attributes are calibrated (floor_area and
# property_age both have an optimal multiplier of exactly 1.00); it is only our
# own subjective scoring that is over-trusted.
#
# ⚠ Two independent lines of evidence agree. Blinding the SUBJECT to
# photo-derived attributes entirely — which is what an off-market home actually
# looks like — also improved MAE (10.22% -> 9.93%). Removing the noise and never
# having it are the same thing.
#
...
def apply_retired_adjustments(adjustment_result):
    """Zero the dollar effect of retired adjustments, keeping them visible.

    Mutates and returns `adjustment_result`. The per-attribute entry keeps its
    subject/comp values and gains `retired: True` with the reason, so the
    receipts can still show "we compared this, and it did not move the price".
    """
    if not adjustment_result:
        return adjustment_result
    adjustments = adjustment_result.get('adjustments') or {}
    removed = 0
    for name in _RETIRED_ADJUSTMENTS:
        entry = adjustments.get(name)
        if not entry:
            continue
        removed += entry.get('dollars') or 0
        entry['dollars'] = 0
        entry['retired'] = True
        entry['retired_reason'] = 'measured to add error (2026-08-07 ablation)'
    if removed:
        comp_price = adjustment_result.get('adjusted_price', 0) - (
            adjustment_result.get('total_adjustment') or 0)
        adjustment_result['total_adjustment'] = (
            adjustment_result.get('total_adjustment') or 0) - removed
        adjustment_result['adjusted_price'] = round(
            adjustment_result['adjusted_price'] - removed)
        if comp_price:
            adjustment_result['total_adjustment_pct'] = round(
                adjustment_result['total_adjustment'] / comp_price, 3)
    return adjustment_result


def suburb_calibration_factor(suburb_key):
    """Multiplier correcting this suburb's measured systematic low bias."""
    return _SUBURB_CALIBRATION.get((suburb_key or '').strip().lower(), 1.0)
...
def precompute_property_valuation(db, subject_doc, listings_coll, sold_by_suburb,
    ...
    # Attached stock hiding behind a house address (added 2026-08-07).
    # The unit-number pattern above misses freehold townhouses entirely.
    # 24 Brooklyn Crescent, Robina is the worked case: property_type "House",
    # is_strata_title False, Freehold tenure, no unit number, and a cadastral
    # parcel measuring 5 m x 26 m. It is a townhouse in a row, and we valued it
    # 56% high. Two signals catch it, and neither needs new data:
    #
    #   1. QLD plan prefix GTP / BUP / CTS / SUP = community or building-units
    #      title. 27 of 990 sold "Houses" sit on one. RP and SP are freehold and
    #      prove nothing either way, so plan type alone is not sufficient.
    #   2. Floor-to-land ratio. A detached house needs setbacks, a driveway and
    #      some yard, so it cannot approach 1.0. Across 833 sold houses p50 is
    #      0.32 and p90 is 0.60; 0.70 leaves clear air. 24 Brooklyn is 0.93.
    #      This also catches genuine data errors (495 m2 of floor on 495 m2 of
    #      land) — both classes belong out of a detached-house valuation.
    _plan = (subject_doc.get('PLAN') or '').strip().upper()
    subject_is_strata_plan = bool(re.match(r'^(GTP|BUP|CTS|SUP)', _plan))
    _ftl_land = resolve_land_size(subject_doc) or resolve_numeric(subject_doc.get('lot_size_sqm'))
    _ftl_floor = resolve_floor_area(subject_doc)
    subject_floor_to_land = (
        (_ftl_floor / _ftl_land) if _ftl_land and _ftl_floor and _ftl_land > 0 else None)
    subject_is_attached = bool(
        subject_is_unit
        or subject_is_strata_plan
        or subject_doc.get('is_strata_title')
        or (subject_floor_to_land is not None and subject_floor_to_land > 0.70)
    )
    ...
    elif subject_is_attached and prop_type == 'House':
    ...
    for point_list in [comparable_points, recent_points]:
        for pt in point_list:
            ...
            # Retired 2026-08-07: kitchen / renovation / renovation_quality are still
            # computed and shown, but no longer move the price. Applied HERE, before
    ...
    # range with it so the band stays centred on the corrected estimate.
    ...
    # Attached dwellings get a direction, never a figure — the method is built for
    # detached houses and measured MAE 18.0% against 10.3% on attached stock that
    # slipped through. Same treatment as an envelope breach: comps and adjustments
    # are kept, the number and the range are withheld.
    if subject_is_attached and prop_type == 'House':
        directional_only = True
        envelope_breach = envelope_breach or 'attached_dwelling'
