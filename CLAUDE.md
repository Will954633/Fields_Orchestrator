...
- **Range (updated 2026-08-08 — the flat ±12% band was RETIRED 2026-08-07; do not describe it as current):** a **per-suburb empirical 80% band**, derived from a backtest over **n=581** real sales — Varsity Lakes **±11.2%**, Robina **±12.2%**, Burleigh Waters **±14.0%**. Four in five sales land inside it. It is a **measured coverage band, NOT a confidence interval**, so **never call it a "confidence range" / "high-confidence range" / "90% confidence range"**. Emitted as `valuation_data.confidence.range_basis` (`kind="empirical_80_band"`, `measured_on`); source `precompute_valuations.py:2141-2185` (`_band_for_suburb`). ⚠ The retired flat ±12% / "contains the sale 61% of the time" / "a 90% band needs ±26.4%" figures describe the **old** method — they are false of production and must not be quoted or propagated into prompts or copy.
...
- **⚠ Accuracy figures (governing, 2026-08-08):** always run the backtest with `--price-filter none` for off-market work. The default `sale` anchor prunes comparables using the subject's own sale price — target leakage. Scoped to the envelope, all three suburbs: **MAE 8.05%, median 6.44%, within-10% 69%** (n=581) — source `16_Valuation/accuracy/2026-08-08-figures.md`. The older **10.5% / 8.2% / 59% (Robina, n=278)** figures are superseded; do not quote them.
