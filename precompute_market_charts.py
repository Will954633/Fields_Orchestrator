...
def _get_listing_status_sold(gc_db, suburb_collection_name, start_date_str, end_date_str):
    """
    ...
    try:
        ...
        for doc in coll.find(
            ...
            try:
                ...
                ptype = (doc.get("scraped_data", {}).get("features", {}).get("property_type")
            ...
            try:
                ...
                for doc in coll.find(
                    ...
                    if not sd:
                        continue
                    try:
                        dt = datetime.strptime(str(sd)[:10], "%Y-%m-%d") if not isinstance(sd, datetime) else sd
                        q = (dt.month - 1) // 3 + 1
                        ptype = (doc.get("scraped_data", {}).get("features", {}).get("property_type") or doc.get("property_type", "Unknown"))
                        results.append({"date": dt, "days_on_market": doc.get("days_on_market"), "price": doc.get("sold_price"), "property_type": ptype, "quarter": f"{dt.year}-Q{q}", "year": dt.year})
                    except (ValueError, TypeError):
                        continue
                return results
...
def precompute_suburb_charts(gc_db, sold_db, suburb_collection_name):
    """
    Pre-compute all 4 chart types for a single suburb.
...
def main():
    ...
    for i, suburb_name in enumerate(suburb_collections):
        results = precompute_suburb_charts(gc_db, sold_db, suburb_name)
        if results:
