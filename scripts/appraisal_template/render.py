...
{% endfor %}
    ...
    <div class="campaign-model" style="background:#fdf3ec; border-radius:3px; padding:3mm 5mm; margin: 4mm 0 2mm; font-size:9.5pt; line-height:1.45; font-style:italic; color:#2c2924;">
      ...
      Based on recent campaign benchmarks, a property like {{ subject.short_address }} would be marketed toward approximately <strong style="font-style:normal; color:#B76749;">{{ '{:,}'.format(s04.campaign_model.impressions_low|int) }}–{{ '{:,}'.format(s04.campaign_model.impressions_high|int) }}</strong><span style="font-style:normal;"> targeted impressions</span>, with the aim of generating <strong style="font-style:normal; color:#B76749;">{{ s04.campaign_model.engagements_low }}–{{ s04.campaign_model.engagements_high }}</strong><span style="font-style:normal;"> deep engagements</span> and <strong style="font-style:normal; color:#B76749;">{{ s04.campaign_model.inspections_low }}–{{ s04.campaign_model.inspections_high }}</strong><span style="font-style:normal;"> inspections</span>. The objective is not exposure for its own sake — it is enough targeted reach to find the right buyer, and enough inspection volume to create competition.
    </div>
