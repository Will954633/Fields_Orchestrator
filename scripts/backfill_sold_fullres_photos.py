...
def _localise_one(bucket: str, suburb: str, doc_id: str, idx: int, url: str) -> bool:
    """Download one full-res image and write photos/<NN>.jpg + thumbs/<NN>.jpg
    under the given bucket ("sold" or "for_sale"). The flat NN.jpg naming is what
    the resolver's _comp_local_photos reads (top-level files only)."""
    data = _fetch(url)
    if not data:
        return False
    nn = f"{idx:02d}"
    full_name = f"{bucket}/{suburb}/{doc_id}/photos/{nn}.jpg"
    ok_full = blob_storage.upload(CONTAINER, full_name, data, content_type="image/jpeg")
    thumb = _make_thumb(data)
    if thumb:
        blob_storage.upload(CONTAINER, f"{bucket}/{suburb}/{doc_id}/thumbs/{nn}.jpg",
                            thumb, content_type="image/jpeg")
    return bool(ok_full)


def _already_done(bucket: str, suburb: str, doc_id: str) -> int:
    """How many flat full-res images are already localised on disk for this home.
    Counts only top-level files in photos/ — legacy nested date subdirs (the old
    for_sale layout) are ignored, so a home with only nested photos is re-localised
    into the clean flat structure the resolver reads."""
    root = blob_storage._local_root() / CONTAINER / bucket / suburb / doc_id / "photos"
    try:
...
def process_home(db, suburb: str, doc: dict, cap: int, dry_run: bool, bucket: str) -> dict:
    ...
    with ThreadPoolExecutor(max_workers=PHOTO_WORKERS) as ex:
        futs = {ex.submit(_localise_one, bucket, suburb, doc_id, i, u): i for i, u in enumerate(urls)}
    ...
    if done:
        db[suburb].update_one(
            {"_id": doc["_id"]},
            {"$set": {
                "fullres_localized": True,
                "fullres_count": done,
                "fullres_at": datetime.now(timezone.utc).isoformat(),
            }},
        )
...
def main():
    ...
    ap.add_argument("--status", choices=["sold", "for_sale"], default="sold",
                    help="which listings to localise (drives both the query and the blob bucket)")
    ...
    bucket = args.status  # "sold" or "for_sale" — also the blob path bucket

    log.info(f"=== backfill start: status={args.status} suburbs={suburbs} cap={args.cap} "
             f"ids={len(id_filter) or 'all'} dry_run={args.dry_run} ===")
    totals = {"done": 0, "skip": 0, "no_source": 0, "would_do": 0, "images": 0}
    for suburb in suburbs:
        q = {"listing_status": args.status, "property_images_original": {"$exists": True, "$ne": []}}
        if id_filter:
        ...
        homes = list(cur)
        log.info(f"[{suburb}] {len(homes)} {args.status} homes to consider")
        for n, doc in enumerate(homes, 1):
            r = process_home(db, suburb, doc, args.cap, args.dry_run, bucket)
