#!/usr/bin/env python3
"""Re-scrape the Robina cadastral records that were just postcode-normalised.
...
Run:
    python3 scripts/rescrape_postcode_fixed_robina.py            # full
    python3 scripts/rescrape_postcode_fixed_robina.py --limit 50 # bounded
"""
...
# Logging — line-buffered to stdout so background runs stream nicely
logging.basicConfig(
    level=logging.INFO,
...
def main() -> int:
    ...
    # Strict scope: postcode was normalised AND no Domain hero image yet
    query = {
        "LOCALITY": "ROBINA",
