...
def _format_inputs(
    ...
    lines.append("")
    lines.append("SOLD COHORT PREMIUMS (last 24 months, suburb catchment):")
    for p in cohort_premiums or []:
        if not p.get("premium_pct"):
            continue
    ...
    if valuation_range:
        lo, hi = valuation_range.get("low"), valuation_range.get("high")
        if lo and hi:
            lines.append("")
            lines.append(f"WORKING VALUATION RANGE: ${lo:,} – ${hi:,} (engine pre-consultant)")
