#!/usr/bin/env python3
"""
cycle_state.py — self-pacing scheduler state, parametrised by --job (SEO, and future domains).
...
import os as _os
JOB = _os.environ.get("PACER_JOB", "seo")
COLL = f"rl_{JOB}_cycle_state"
SID = "state"
...
def show():
    d = _roll_day(_load())
    cap = int(d.get("max_cycles_per_day", MAX_CYCLES_PER_DAY))
    print(f"{COLL.replace(chr(34),chr(0)) and JOB} cycle state: {d['cycles_today']}/{cap} cycles today ({d['cycles_today_date']})")
...
def main():
    ...
    ap.add_argument("--job", default=None, help="domain name → rl_<job>_cycle_state")
