...
═══ STEPS ═══
1. GATHER: run the three sensors (above); read their `latest` docs (`rl_geo_signal`, `rl_reward_ledger`, `rl_personalization_policy`) + `rl_geo_actions` (what past cycles already did — do NOT repeat) + the last 1-2 geo cycle docs (now filed in weekly/daily subfolders — `find 16_General_Reinforcement_Learning/cycles -name 'geo_cycle_*.md' | sort | tail -2`). Per engine: users, conversions, conv-rate vs base, lift, weekly trend, DORMANT flags, landing pages. **Also read + action your open conductor directives:** `python3 conductor_state.py directives --domain geo` (these are durable instructions the conductor issued to you; do them, then close with `conductor_state.py done --id <id>`).
