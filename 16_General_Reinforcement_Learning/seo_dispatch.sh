#!/usr/bin/env bash
# seo_dispatch.sh — cheap poller that gives the SEO cycle SELF-PACING.
...
LOG=/home/fields/Fields_Orchestrator/logs/seo_dispatch.log
...
DECISION=$(PACER_JOB=seo python3 "$DIR/cycle_pacer.py" --job seo --claim 2>&1); RC=$?
echo "[$STAMP] $DECISION" >> "$LOG"
if [ "$RC" -eq 0 ]; then
  echo "[$STAMP] launching seo_cycle.sh" >> "$LOG"
  "$DIR/seo_cycle.sh"
fi
exit 0
