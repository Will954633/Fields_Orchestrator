...
class TaskManager:
    ...
    async def _execute(self, task_id: str, prompt: str,
        ...
        worker_system = (
            ...
            f"BRAINS (curated intelligence graphs — prefer these for 'what have WE decided/built/learned' "
            f"and coaching best-practice; results cite unit ids): "
            f"python3 scripts/samantha/brain_search.py \"query\" --brain 3  (internal ops: fix-logs, CEO memory, "
            f"articles, strategy, ad decisions, seller book) | --brain 1  (external: coaching corpus + books). "
            f"Fast, no LLM cost. Add --deep for the full synthesised-answer command (slower, costs).\n\n"
            f"SAVE TO KB: You can save important content (emails, meeting notes, decisions, analysis) to the KB:\n"
            f"  python3 scripts/save-to-kb.py --file /path/to/doc.md                    # Save a file (AI classifies)\n"
            f"  python3 scripts/save-to-kb.py --text \"content\" --title \"Title\" --category strategy  # Save text\n"
            f"  echo \"content\" | python3 scripts/save-to-kb.py --stdin --title \"Title\" --category conversations  # Pipe\n"
            f"  Add --tags \"tag1,tag2\" for custom tags. Add --skip-ai for fast save without AI extraction.\n"
            f"  Categories: book, strategy, marketing, code, financial, operational, meeting_notes, conversations, general, project\n"
            f"  Saves locally + uploads to Azure Blob automatically.\n\n"
            f"EMAIL ATTACHMENTS TO KB: Download email attachments and save to KB in one step:\n"
            f"  python3 scripts/email-attachment-to-kb.py <message_id> [--category TYPE] [--tags \"tag1,tag2\"] [--skip-ai]\n"
            f"  Supports: PDF, DOCX, TXT, MD, XLSX. Downloads via Graph API, classifies, chunks, saves + uploads.\n\n"
