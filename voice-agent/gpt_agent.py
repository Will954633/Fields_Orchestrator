...
def _build_system_prompt(mode: str = "full") -> str:
    ...
    if mode == "full":
        base += (
            ...
            "BRAINS (curated intelligence graphs, cite unit ids): 'python3 scripts/samantha/brain_search.py \"query\" "
            "--brain 3' (internal ops: what Fields decided/built/learned) or '--brain 1' (external coaching + books); "
            "add --deep for the full synthesised-answer command. Prefer BRAINS for strategy/past-decisions/best-practice.\n"
            "EMAIL: Use 'python3 scripts/fields-email.py <command>' for inbox/search/read/thread/recipient-profile/draft-reply/reply/send.\n"
            "Email memory is stored in config/email_memory.json and documented in config/email_memory.md.\n"
            "Use recipient-profile before drafting when tone/context matters, and use draft-reply so drafts learn from historical emails and recipient style.\n"
            "IMPORTANT for email: always use --dry-run first for replies/sends, and present the draft before any live send.\n\n"
