...
2. **End-state / honesty (REVISED 2026-07-30, Will):** the newsletter idea is DROPPED — overkill for a lab. The only honesty rule that survives is **no forward-delivery promise** (never "we'll email/send you X" unless we will). Everything a user gives is either (a) a measured micro-conversion, or (b) a gate that **unlocks immediate on-page value right there** (e.g. give email → the next step's illustrative numbers appear instantly). This makes **post-email / post-address engagement** — the core warm-up thesis ("does giving contact info predict deeper engagement → eventual call?") — testable with zero delivery obligation. Terminal state is a neutral honest end (no future promise). No newsletter, no waitlist framing, no delivery dependency.
...
3. ~~Brisbane market-newsletter~~ — **DROPPED** (see §11.2 revision; no delivery dependency).
4. **First approved template** (§8 gate) — progressive multi-step (name → email → address), each ask unlocking immediate on-page value, neutral honest terminal. Preview link + spec to Will, stop for approval.
