...
function parseArgs() {
  ...
  const o = {
    ...
    crf: 21,
    out: null,
    html: null,   // which built page to drive; defaults to ./index.html
...
async function main() {
  const o = parseArgs();
  const indexPath = path.resolve(HERE, o.html || "index.html");
  if (!fs.existsSync(indexPath)) {
    console.error(`${indexPath} missing — run: python3 build_reveal.py`);
    process.exit(1);
  }
  ...
  const frameDir = o.stills > 0 ? null : fs.mkdtempSync(path.join(os.tmpdir(), "fields-reveal-"));
  try {
    await render(o, url, frameDir, indexPath);
  } finally {
...
async function render(o, url, frameDir, indexPath) {
  ...
  try {
    ...
    if (o.stills > 0) {
      ...
      for (let i = 0; i < o.stills; i++) {
        ...
        const f = path.join(path.dirname(indexPath),
          `${path.basename(indexPath, ".html")}_still_${o.mode}_${String(i).padStart(2, "0")}.png`);
