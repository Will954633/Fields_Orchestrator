...
3. **But the loop cannot learn anything today.** Two hard constraints gate it, and both must be addressed *before* a learning loop is meaningful:
   ...
   - **More volume is data-gated, not quota-gated.** You can't reach "whole-GC $1M–$2M houses" by *filtering* — that bracket is only **~1,601 houses whole-GC vs ~1,573 already in the 3 core suburbs**, and everything outside core is bare cadastral skeleton. Expanding means *acquiring* data (PropRadar), and the cheap lever is the per-suburb **sold feed**, not per-address lookups. Google index "quota" is a non-issue; the real ceiling is crawl-budget × domain authority × page quality.
...
### Constraint B — volume is data-gated, and the smart expansion is quality-first
More trials/week is the throttle on learning rate. It **cannot** come from filtering the DB (the $1–2M bracket is ~1,600 whole-GC and non-core is skeleton). It must come from **acquiring sale-history data** for adjacent suburbs, cheaply:
- **PropRadar blind per-address enrichment is infeasible** — ~3–4 calls/address × ~84K southern non-core ≈ **250K–340K calls = 4–5 years of the Hobby ~5,000/mo budget.**
- **The cheap lever is the per-suburb sold feed** — `GET /suburbs/QLD/{suburb}/sold?property_type=House&months=60` returns named `address`+`sold_price`+`sold_date`+beds/baths at ~10–25 calls/suburb. **Seeding all 22 southern non-core suburbs ≈ 250–600 calls — one month's Hobby budget.** A recorded sale is *exactly* the eligibility signal driving the current 90% indexation.
- **Size the universe first for ~free:** one `GET /suburbs/QLD/{suburb}` call × 85 suburbs = 85 calls tells us which suburbs even have house medians in the target band before spending.
- **`POST /valuations` (2/day cap) is irrelevant** — we compute valuations with our own engine; PropRadar supplies sale history + attributes only.
- **Tier upgrade that would matter:** **Starter** unlocks `/properties/recently-modified` (a daily "who just listed / cut price / sold" feed = ongoing new-page trickle **and** the strongest inbound seller-intent trigger) and `POST /properties/bulk`. Not needed for a one-time seed; worth it for continuous freshness.
...
**Phase 1 — Lift volume the smart way (coverage; ~weeks, ~hundreds of PropRadar calls).**
Size all 85 suburbs (~85 calls); harvest per-suburb `/sold?property_type=House` for 2–4 highest-median **adjacent** suburbs (e.g. Palm Beach, Mermaid Waters, Miami, Reedy Creek); mint off-market pages **only for addresses with a recorded sale, prioritising $1–2M**; densify internal links among off-market/property pages to spend existing crawl budget better; **watch GSC 2–4 weeks.** *Exit test:* the adjacent lap indexes and pulls impressions at the core rate → scale suburb-by-suburb; if it piles into "discovered-not-indexed," that's the authority ceiling and effort shifts to links/PR, not more pages.
...
2. **Coverage scope:** happy to **expand quality-first into 2–4 adjacent southern suburbs via the PropRadar sold feed** (cheap), rather than chase the whole-GC "$1–2M houses" filter (only ~1,600 houses, non-core is skeleton)? And confirm we **skip the other-city lap**?
3. **PropRadar tier:** is the ongoing `/properties/recently-modified` feed (Starter tier) worth it — it doubles as the strongest **inbound seller-intent trigger** (who just listed/cut price/sold), not only a page-seeding tool?
4. **Arm strategy:** collapse to a **single canonical deck** and A/B *cards within it*, or keep `rich` vs `ladder_dark` as competing whole-deck arms (harder to instrument comparably)?
5. **True-reward definition:** for the sparse reward, is the target `offmarket_qualify` (in-deck seller signal), a lead submit, or a **booked call / posted appraisal**? This sets what the loop actually optimises.
6. **Cadence:** daily observe + weekly decide (recommended), or push for twice-daily once Phase 1 lifts volume?
...
- PropRadar (Hobby ~5,000 calls/mo): per-suburb sold feed ~10–25 calls/suburb; **seed 22 southern suburbs ≈ 250–600 calls**; blind per-address ≈ 250K–340K calls (infeasible).
