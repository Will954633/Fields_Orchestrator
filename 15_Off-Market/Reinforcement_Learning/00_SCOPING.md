...
**Phase 1 — Lift volume via whole-population timeline enrichment (coverage; Bright Data corpus + PropRadar selection).**
Rank the 26 southern-to-middle suburbs by $1–2M house stock (one PropRadar snapshot call each); **Bright Data Domain-scrape full property timelines** for the cadastral address population of the top suburbs (reusing `domain_fetch.py` + the step-12 timeline builder), starting with 2–4 highest-value (e.g. Palm Beach, Burleigh Heads, Mermaid Waters, Reedy Creek); mint rich off-market pages per address and gate on the timeline for off-market eligibility; densify internal links among off-market/property pages to spend existing crawl budget better; stand up the PropRadar `/recently-modified` daily feed as an ongoing inbound-intent trigger; **watch GSC 2–4 weeks.** *Exit test:* the first wave indexes and pulls impressions at the core rate → scale suburb-by-suburb through the 117K band; if it piles into "discovered-not-indexed," that's the authority ceiling and effort shifts to links/PR, not more pages. Can run **in parallel with Phase 0** (backend scrape vs frontend instrumentation are independent) but the *loop* needs both.
...
2. **Coverage scope & pace:** expand via **Bright Data timeline enrichment across the 117K southern-to-middle band (incl Nerang), in watched waves highest-value-suburb-first**? Confirm the one-time Bright Data spend (~$175–350) is fine, and that we **skip the other-city lap**.
3. **PropRadar Starter roles:** agree it's for **suburb-selection + the `/recently-modified` inbound-intent feed + verification**, not bulk enrichment (the scraper does that cheaper/faster)? Worth standing the intent feed up now as its own seller-signal, independent of the RL loop?
